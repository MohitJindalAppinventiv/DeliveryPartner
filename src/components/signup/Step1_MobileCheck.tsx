...
// src/features/signup/Step1_EmailCheck.tsx
import { useDispatch, useSelector } from 'react-redux';
import { useState } from 'react';
// import axios from 'axios';
...
export default function Step1EmailCheck() {
  ...
  const handleCheckEmail = async () => {
    ...
    try {
      ...
      if (res.data.exists) {
        setError('Email already exists. Try LogIn');
        setShowSignIn(true);
      } else {
        // await axios.post('/api/send-verification-email', { email }); // <== ADD THIS
        dispatch(nextStep());
      }
    } catch (err) {
      setError('Server error. Please try again.');
    ...
    <motion.div 
      ...
      <div className="mb-6">
        ...
        <div className="relative">
          ...
          />
          <Link to={'/login'}>
          <div className='text-blue-400 text-sm' > Already Have a Account? Try Login</div>
          </Link>
        </div>
        {error && (
          <motion.p 
